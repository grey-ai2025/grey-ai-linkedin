...
def register_person_tools(mcp: FastMCP) -> None:
    ...
    async def get_person_profile(
        ...
        Args:
            ...
            sections: Comma-separated list of extra sections to scrape.
                ...
                Available sections: experience, education, interests, honors, languages, contact_info, recent_activity
    ...
    async def get_person_posts(
        ...
        """
        Get recent posts from a person's LinkedIn activity feed.

        Args:
            linkedin_username: LinkedIn username (e.g., "emollick", "williamhgates")
            ctx: FastMCP context for progress reporting
        ...
        try:
            ...
            logger.info("Scraping person posts: %s", linkedin_username)

            browser = await get_or_create_browser()
            extractor = LinkedInExtractor(browser.page)

            await ctx.report_progress(
                progress=0, total=100, message="Starting person posts scrape"
            )
